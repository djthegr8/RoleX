...
namespace RoleX.Modules.Role_Editor
{
    ...
    internal class RoleCreate : CommandModuleBase
    {
        ...
        [DiscordCommand("rcreate", commandHelp = "rcreate <name> <color>", description = "Creates role with given name and color", example = "rcreate WeirdRole #bb86fc")]
            ...
            switch (args.Length)
            {
                case 0:
                    await ReplyAsync("", false, new EmbedBuilder
                    {
                        ...
                        Description = $"The way to use the command is `{await SqliteClass.PrefixGetter(Context.Guild.Id)}rcreate <name> <color?>`",
                        Color = Color.Red
