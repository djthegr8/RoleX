...
namespace Hermes.Utilities.Collector
{
    public static class CollectorsUtils
    {
        private static readonly
            Subject<(Cacheable<IUserMessage, ulong>, Cacheable<IMessageChannel, ulong>, SocketReaction)> ReactionAdded =
                new();

        private static readonly Subject<SocketMessage> MessageReceived = new();
        ...
        public static CollectorController CollectReaction(IChannel channel, Predicate<SocketReaction> predicate,
            Action<EmoteCollectorEventArgs> action, CollectorFilter filter = CollectorFilter.Off)
        {
            return CollectReaction(reaction => channel.Id == reaction.Channel.Id && predicate(reaction), action,
                filter);
        }
        ...
        public static CollectorsGroup CollectReactions<T>(Predicate<SocketReaction> predicate,
            ...
            params (IEmote, Func<T>)[] selectors)
        {
            ...
            foreach (var selector in selectors.ToList())
            {
                ...
                var localPredicate = new Predicate<SocketReaction>(reaction =>
                    reaction.Emote.Equals(selector.Item1) && predicate(reaction));
                var disposable = ReactionAdded.Subscribe(tuple =>
                {
                    try
                    {
        ...
        private static Predicate<IMessage> ApplyFilters(Predicate<IMessage> initial, CollectorFilter filter)
        {
            return filter switch
            {
                ...
                CollectorFilter.IgnoreSelf => message =>
                    message.Author.Id != Program.Client.CurrentUser.Id && initial(message),
        ...
        private static Predicate<SocketReaction> ApplyFilters(Predicate<SocketReaction> initial, CollectorFilter filter)
        {
            return filter switch
            {
                ...
                CollectorFilter.IgnoreSelf => reaction =>
                    reaction.UserId != Program.Client.CurrentUser.Id && initial(reaction),
