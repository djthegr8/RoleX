...
namespace Hermes.Modules.Channel_Permission
{
    ...
    {
        [Alt("pinc")]
        [DiscordCommand("pincount", description = "Fetches the number of pins in the channel",
            commandHelp = "pincount #channel", example = "pincount #media")]
        public async Task PC(params string[] args)
        {
            ...
            loa = loa.Select(x => new Tuple<string, int>(x.Item1,
                pins.Count(k => k.Author.Username + "#" + k.Author.Discriminator == x.Item1))).ToList();
