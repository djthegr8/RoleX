    ...
    public class CommandModuleBase
    {
        ...
        public async Task<GuildEmote> GetEmote(string str, SocketGuild Guild = null)
        {
            ...
            try
            {
                ...
                if (resultString == 0 || await Context.Guild.GetEmoteAsync(resultString) == null) return null;
        ...
        public async Task<SocketGuildUser> GetUser(string user)
        {
            ...
            user = user.ToLower();
            if (Context.Message.MentionedUsers.Any()) return Context.Message.MentionedUsers.First() as SocketGuildUser;
            ...
            return Context.Guild.Users.Any(x => x.ToString().ToLower().StartsWith(user))
                ? Context.Guild.Users.First(x => x.ToString().ToLower().StartsWith(user))
                : Context.Guild.Users.FirstOrDefault(x => x.Nickname != null && x.Nickname.ToLower().StartsWith(user));
        }
        ...
        public async Task<IUser> GetBannedUser(string uname)
        {
            ...
            return regex.IsMatch(uname)
                ...
                : alr.FirstOrDefault(x => x.User.Username.ToLower().Contains(uname.ToLower()))?.User;
        }
        ...
        public static OverwritePermissions GetOP(ChannelPermission cp, PermValue pv, OverwritePermissions eop)
        {
            var x = cp switch
            {
                ...
                ChannelPermission.CreateInstantInvite => eop.Modify(pv),
        ...
        public static Tuple<GuildPermission, bool> GetPermission(string perm)
        {
            ...
            return Enum.TryParse(perm, true, out GuildPermission Gp)
                ? new Tuple<GuildPermission, bool>(Gp, true)
                : new Tuple<GuildPermission, bool>(GuildPermission.AddReactions, false);
        }

        public static Tuple<ChannelPermission, bool> GetChannelPermission(string perm)
        {
            return Enum.TryParse(perm, true, out ChannelPermission Gp)
                ? new Tuple<ChannelPermission, bool>(Gp, true)
                : new Tuple<ChannelPermission, bool>(ChannelPermission.AddReactions, false);
        }

        /// <exception cref="Discord.Net.HttpException" />
        /// Throws when role is everyone role and some ops are run
        public SocketRole GetRole(string role)
        {
            if (role.Length < 3) return Context.Guild.Roles.FirstOrDefault(k => k.Name.ToLower() == role);
            var regex = new Regex(@"(\d{18}|\d{17})");
            ...
            return Context.Guild.Roles.Any(x => x.Name.ToLower().StartsWith(role.ToLower()))
                ? Context.Guild.Roles.First(x => x.Name.ToLower().StartsWith(role.ToLower()))
                : null;
        }
        ...
        public async Task<IUserMessage> ReplyAsync(string message = null, bool isTTS = false, EmbedBuilder embed = null,
            MessageComponent mcom = null, RequestOptions options = null)
        {
            ...
            if (msgcontent.EndsWith("-q")) return null;
            ...
            if (message == null && embed == null) return null;
            // Embed editing time!
