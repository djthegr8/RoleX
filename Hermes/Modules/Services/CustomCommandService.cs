...
namespace Hermes.Modules.Services
{
    ...
    public class CustomCommandService
    {
        ...
        public CustomCommandService(Settings s)
        {
            ...
            var CommandMethods = new Dictionary<MethodInfo, Type>();
            var types = Assembly.GetEntryAssembly() == null
                ? new List<Type>().ToArray()
                : Assembly.GetEntryAssembly().GetTypes();
            var CommandClasses =
                types.Where(x => x.CustomAttributes.Any(x => x.AttributeType == typeof(DiscordCommandClass)));
            ...
            var Commandmethods = types.SelectMany(x =>
                x.GetMethods().Where(y => y.GetCustomAttributes(typeof(DiscordCommand), false).Length > 0).ToArray());
            foreach (var t in Commandmethods) CommandMethods.Add(t, t.DeclaringType);
            ...
            foreach (var item in CommandMethods)
            {
                ...
                var c = new Commands
                {
                    ...
                    RequireUsrPerm =
                        item.Key.CustomAttributes.Any(x => x.AttributeType == typeof(RequiredUserPermissions))
        ...
        public static Dictionary<string, string> Modules { get; set; } = new();
        ...
        public async Task<ICommandResult> ExecuteAsync(SocketCommandContext context, string pref)
        {
            ...
            if (hasN) msgcontent = msgcontent.Replace("-n", "");
            if (!ignoreAlias)
            {
                var alss = await SqliteClass.GuildAliasGetter(context.Guild.Id);
                foreach (var (aliasName, aliasContent) in alss)
                    msgcontent = msgcontent.Split(" ")[0].Replace(aliasName, aliasContent) + " " +
                                 string.Join(" ", msgcontent.Split(" ").Skip(1));
            }
         ...
         param = msgcontent.Split(' ');*/
            ...
            if (commandobj.Any(x => x.Paramaters.Length > 0))
                if (commandobj.Where(x => x.Paramaters.Length > 0).Any(x =>
                    x.Paramaters.Last().GetCustomAttributes(typeof(ParamArrayAttribute), false).Length == 0))
            ...
            param = idegk.ToArray();
            foreach (var cmd in commandobj) results.Add(await ExecuteCommand(cmd, context, param));
        ...
        private async Task<CommandResult> ExecuteCommand(Command cmd, SocketCommandContext context, string[] param)
        {
            if (!(context.User as SocketGuildUser).GuildPermissions.Administrator &&
                CommandModuleBase.devids.All(k => k != context.User.Id))
                if (cmd.perms != null)
            ...
            if (cmd.Paramaters.Length == 0 && param.Length == 0)
                try
                {
                    ...
                    cmd.parent.ClassInstance.GetType().GetProperty("Context")
                        .SetValue(cmd.parent.ClassInstance, context);

                    var d = (Func<Task>) Delegate.CreateDelegate(typeof(Func<Task>), cmd.parent.ClassInstance,
                        cmd.Method);
                    await d();
            ...
            if (cmd.Paramaters.Last().GetCustomAttributes(typeof(ParamArrayAttribute), false).Length > 0)
            {
                ...
                var check = true;
                for (var i = 0; i != cmd.Paramaters.Length; i++)
                {
                    var dp = cmd.Paramaters[i];
                    if (dp.GetCustomAttributes(typeof(ParamArrayAttribute), false).Length > 0)
                    {
                        var arr = param.Skip(i).Where(x => x != "").ToArray();
                        if (arr.Length == 0)
                        {
                            var al = new ArrayList();
                            var destyp = dp.ParameterType.GetElementType();
                ...
                if (check)
                    ...
                    try
                    {
                        var u = context.Guild.GetUser(context.Message.Author.Id);
                        CommandModuleBase.HasExecutePermission = true;
                        if (!currentSettings.AllowCommandExecutionOnInvalidPermissions &&
                        ...
                        var s = (Task) cmd.Method.Invoke(cmd.parent.ClassInstance, parsedparams.ToArray());
                        Task.Run(async () => await s).Wait();
                        if (s.Exception == null)
                            return new CommandResult {Result = CommandStatus.Success, IsSuccess = true};
                        return new CommandResult
                            {Exception = s.Exception.InnerException, Result = CommandStatus.Error, IsSuccess = false};
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex);
        ...
        public class CommandResult : ICommandResult
        {
            ...
            [DefaultValue(false)] public bool MultipleResults { get; set; }
        ...
        /// <summary>
        ///     The Command Result, this contains the <see cref="CommandStatus" /> enum and a <see cref="Exception" /> object if
        ///     there was an error.
        /// </summary>
