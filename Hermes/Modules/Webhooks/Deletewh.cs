...
namespace Hermes.Modules.Webhooks
{
    ...
    public class Deletewh : CommandModuleBase
    {
        ...
        [DiscordCommand("deletewh", commandHelp = "deletewh <webhook-name/id>",
        ...
        public async Task DeleteWh(params string[] args)
        {
            ...
            if (iGTSW.Count == 1)
            {
                ...
                await ReplyAsync("", false, new EmbedBuilder
                {
                    ...
                    Description =
                        $"The webhook `{reqwh.Name}` of channel <#{reqwh.ChannelId}> was deleted successfully!!",
                    Color = Blurple
                }.WithCurrentTimestamp());
            ...
            for (var i = 0; i < iGTSW.Count; i++)
            {
                var rw = iGTSW[i];
                emb.AddField($"{i + 1}) " + rw.Name,
