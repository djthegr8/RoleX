...
namespace Hermes.Modules.Webhooks
{
    ...
    public class Showwh : CommandModuleBase
    {
        ...
        [DiscordCommand("showwh", commandHelp = "deletewh <#channel>",
            description = "Deletes the first webhook of given name or ID", example = "deletewh MyWebhook")]
            ...
            if (args.Length == 0)
            {
                if (allGWH.Count == 0)
                {
                    await ReplyAsync("", false, new EmbedBuilder
                    {
                        ...
                        Description =
                            $"If you want to make a Webhook, run `{await SqliteClass.PrefixGetter(Context.Guild.Id)}addwh`",
                        Color = Color.Red
