...
namespace Hermes.Modules.General
{
    ...
    {
        [DiscordCommand("botsuggest", commandHelp = "botsuggest <suggestions-for-Hermes-to-improve>", description = "Tells the devs about a suggestion", example = "botsuggest new weird command")]
        public async Task SuggestCommand(params string[] args)
        {
            if (args.Length == 0)
            {
                await ReplyAsync("", false, new EmbedBuilder
                {
                    ...
                    Description = $"The way to use the command is `{await SqliteClass.PrefixGetter(Context.Guild.Id)}botsuggest <suggestions-for-Hermes-to-improve>",
                    Color = Color.Red
            ...
            await (Program.Client.GetChannel(755640198662062220) as SocketTextChannel).SendMessageAsync("", false,
                new EmbedBuilder
                {
                    ...
                    Color = Blurple
                }.AddField($"By {Context.User}", $"**ID**: {Context.User.Id}\n**Mention**: {Context.User.Mention}\n**Server ID**: {Context.Guild.Id}").WithCurrentTimestamp().Build());
