...
namespace Hermes.Modules.General
{
    ...
    internal class GraphChartOrSmth : CommandModuleBase
    {
        // Pens used to outline pie slices.
        private readonly Pen[] SlicePens = {Pens.Black};
        ...
        public async Task GraphChartOrSmthCommand(params string[] args)
        {
            ...
            await messageChannelWhichMightActuallyChangeLMAO.SendFileAsync("chart.jpeg",
                $"The top 3 in this channel are ~ \n🥇 {sf.Item1} - {sf.Item2}\n" +
            ...
            List<Tuple<string, ulong>> lis, Brush label_brush, Font label_font, float total, IChannel chnl)
        {
            ...
            for (var i = 0; i < values.Length; i++)
            {
                ...
                gr.DrawString(lis[i].Item1 + $" - {Convert.ToInt32(100 * (lis[i].Item2 / total))}%", label_font,
                    label_brush, new PointF(1140, 100 + 60 * i));
