...
namespace Hermes.Modules.General
{
    ...
    {
        [DiscordCommand("whois", description = "Shows information about the mentioned user",
            commandHelp = "whois <@user>", example = "whois DJ001")]
        public async Task WhoIs(params string[] user)
        {
            ...
            else
            {
                ...
                userAccount = userGuildAccount == null
                    ? null
                    : await Program.Client.Rest.GetUserAsync(userGuildAccount.Id);
            }

            if (userAccount == null)
            {
                if (ulong.TryParse(user[0], out var ide)) userAccount = await Program.Client.Rest.GetUserAsync(ide);
                if (userAccount == null)
                {
            ...
            )
            {
                ...
                foreach (var gld in dry.Take(5))
                {
                    ...
                    mutualServers += $"{(string.IsNullOrEmpty(gUser.Nickname) ? "" : $"`{gUser.Nickname}` in ")}" +
                                     $"**{gld.Name}** ({gld.Id})\n";
                }
            ...
            var stats = $"{(userGuildAccount == null ? "" : $"Nickname: {userGuildAccount.Nickname ?? "None"}\n")}" +
                        $"Id: {userAccount.Id}\n" +
            ...
            var whois = new EmbedBuilder
            {
                Author = new EmbedAuthorBuilder
                {
                    Name = userAccount + (Program.Client.GetUser(userAccount.Id) != null
                        ? " (" + Program.Client.GetUser(userAccount.Id).Status + ")"
                        : ""),
                    IconUrl = userAccount.GetAvatarUrl()
                },
                Color = Blurple,
                Description = "**Stats**\n" + stats,
                Fields = userGuildAccount == null
