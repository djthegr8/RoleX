...
namespace Hermes.Modules.General
{
    ...
    internal class Reminder : CommandModuleBase
    {
        ...
        [DiscordCommand("reminder", commandHelp = "reminder <time> <reason>`\n`reminder list`\n`reminder remove <id>",
            example = "reminder 10m Think about life",
            description = "Reminds the user to do something after specified time!")]
        public async Task ReminderCommand(params string[] args)
        {
            ...
            if (args[0] == "remove" || args[0] == "-")
            {
                if (args.Length == 1)
                    await ReplyAsync(embed: new EmbedBuilder
                    {
                        ...
                        Description =
                            $"Command Syntax: `{await SqliteClass.PrefixGetter(Context.Guild.Id)}reminder remove <id>`\nYou can find the ID using list.",
                        Color = Color.Red
                    }.WithCurrentTimestamp());
                var cudFind = false;
                var lor = await SqliteClass.GetReminders(
                    $"SELECT * FROM reminders WHERE UserID = {Context.User.Id} AND Finished IS 0;");
            ...
            var isValidTime = args[0].Last() switch
            {
                ...
                _ => false
            } && int.TryParse(string.Join("", args[0].SkipLast(1)), out var _);
            if (!isValidTime)
            {
                await ReplyAsync("", false, new EmbedBuilder
                {
                    ...
                    Description =
                        $"Couldn't parse `{args[0]}` as time, see key below\n```s => seconds\nm => minutes\nh => hours\nd => days```",
                    Color = Color.Red
                }.WithCurrentTimestamp());
                return;
            }

            if (int.TryParse(string.Join("", args[0].SkipLast(1)), out var timezar))
            {
            ...
            await SqliteClass.AddReminder(new SqliteClass.Reminder
            {
                UserId = Context.User.Id, TimeS = DateTime.UtcNow.Add(ts),
                Reason = args.Length > 1 ? string.Join("", string.Join(' ', args.Skip(1)).Take(25)) : "Not given"
            });
            await ReplyAsync("", false, new EmbedBuilder
            {
                Title = "Reminder added!",
                Description =
                    $"We shall remind you after `{ts.Days}`d `{ts.Hours}`h, `{ts.Minutes}`m and `{ts.Seconds}`s",
                Color = Blurple
            });
        }
    }
