...
namespace Hermes.Modules.General
{
    ...
    public class Alttime : CommandModuleBase
    {
        ...
        [DiscordCommand("alttime", commandHelp = "alttime num_months",
            description = "Sets the number of months for flagging as alt", example = "alttime 4")]
        public async Task Alttimes(params string[] args)
        {
            if (args.Length == 0)
            {
                await ReplyAsync("", false, new EmbedBuilder
                {
                    ...
                    Description =
                        $"We will flag an account as an alt if it's {await AltTimePeriodGetter(Context.Guild.Id)} months or younger on Discord.",
                    Color = Blurple,
            ...
            if (!ushort.TryParse(args[0], out var t) || t > 12)
            {
                await ReplyAsync("", false, new EmbedBuilder
                {
            ...
            await ReplyAsync("", false, new EmbedBuilder
            {
                ...
                Description =
                    $"We will now flag an account as an alt if it's {await AltTimePeriodGetter(Context.Guild.Id)} months or younger on Discord",
                Color = Blurple,
                Footer = new EmbedFooterBuilder
                {
